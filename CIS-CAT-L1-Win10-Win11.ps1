...
function Invoke-Firewall {
    ...
    foreach ($p in @("Domain","Private","Public")) {
        $rk = "HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\${p}Profile"
        Set-RegValue $rk "EnableFirewall"       1 "DWord" "9.$p.1" "Firewall $p Profile: On"
        Set-RegValue $rk "DefaultInboundAction" 1 "DWord" "9.$p.2" "Firewall $p: Block inbound by default"
        Set-RegValue $rk "DefaultOutboundAction" 0 "DWord" "9.$p.3" "Firewall $p: Allow outbound by default"
        Set-RegValue $rk "DisableNotifications" 0 "DWord" "9.$p.4" "Firewall $p: Show notifications"
        Set-RegValue "$rk\Logging" "LogDroppedPackets" 1 "DWord" "9.$p.5" "Firewall $p: Log dropped packets"
        Set-RegValue "$rk\Logging" "LogSuccessfulConnections" 0 "DWord" "9.$p.6" "Firewall $p: Log successful connections: Disabled"
        Set-RegValue "$rk\Logging" "LogFilePath" "%SystemRoot%\System32\logfiles\firewall\pfirewall.log" "ExpandString" "9.$p.7" "Firewall $p: Log file path"
        Set-RegValue "$rk\Logging" "LogFileSize" 16384 "DWord" "9.$p.8" "Firewall $p: Log file size: 16384 KB"

        # Also configure via cmdlet for immediate effect
        try {
